...
static int add_headset_ag(sdp_session_t *session, svc_info_t *si)
{
	...
	// SS_BLUEZ_BT +kjh 110422
	// remove unnecessary attribute at HSP
...
static int add_handsfree_ag(sdp_session_t *session, svc_info_t *si)
{
	...
	uint16_t u16 = 0x17;
#ifdef ANDROID
	// SS_BLUEZ_BT +chunho 110406
	// supported feature change
	//u16 = 0x07;
	u16 = 0x03; // remove Voice recognition function
#endif
	sdp_data_t *channel, *features;
	uint8_t netid = si->network ? si->network : 0x01; // ???? profile document
	sdp_data_t *network = sdp_data_alloc(SDP_UINT8, &netid);
...
static int add_opush(sdp_session_t *session, svc_info_t *si)
{
	...
	sdp_data_t *channel;
#ifdef ANDROID
	// SS_BLUEZ_BT +chunho 110407
	// Add vCal, vNote
	//uint8_t formats[] = { 0x01, 0x02, 0xff };
	uint8_t formats[] = { 0x01, 0x02, 0x03, 0x05, 0xff };
#else
