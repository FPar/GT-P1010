...
// SS_BLUEZ_BT +kjh
// 2sec is too short, It can make race condition in some headset 
				...
				void *user_data)
{
	...
	if (new_state == AVDTP_SESSION_STATE_CONNECTED) {
		// BEGIN SS_BLUEZ_BT +kjh
		// 2sec is too short, It can make race condition in some headset
		// First, local device waits avrcp connection from remote. after timeout, local device will try. to avoid race condition
		/*
