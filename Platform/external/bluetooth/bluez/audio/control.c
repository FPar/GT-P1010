...
// BEGIN SS_BLUEZ_BT +Chris 2011.05.11
// set response value as "CTYPE_NOT_IMPLEMENTED" if there is no match on keymap
/*
...
static gboolean handle_panel_passthrough(struct control *control,
					const unsigned char *operands,
					int operand_count)
// END SS_BLUEZ_BT +Chris 2011.05.11					
{
	const char *status;
	int pressed, i;

	if (operand_count == 0)
		// SS_BLUEZ_BT +Chris 2011.05.11	
		//return;
		return TRUE;
...
// set response value as "CTYPE_NOT_IMPLEMENTED" if there is no match on keymap			
		...
		if (handle_panel_passthrough(control, operands, operand_count) == FALSE) {
			DBG("handle_panel_passthrough is FALSE ");			
			avrcp->code = CTYPE_NOT_IMPLEMENTED;	
		}
// END SS_BLUEZ_BT +Chris 2011.05.11		
	} else if (avctp->cr == AVCTP_COMMAND &&
...
// BEGIN SS_BLUEZ_BT +YHS 2011.04.18
// Added for debug log.
	DBG("AVRCP: Audio device name: %s", name);
// END SS_BLUEZ_BT  
