...
//BEGIN SS_BLUEZ_BT  (+neo 2011. 04. 14)	
    dev->name[MAX_NAME_LENGTH-1] = '\0';
//END SS_BLUEZ_BT

// SS_BLUEZ_BT +YHS 110525
// For avoid a blanked device name.
/*
	write_local_name(bdaddr, (char *) dev->name);
*/
	if(strlen(dev->name)){
		write_local_name(bdaddr, (char *) dev->name);
		DBG("save device name in config: %s", dev->name);
	}
// END SS_BLUEZ_BT
...
static int adapter_setup(struct btd_adapter *adapter, const char *mode)
{
	...
	if (read_local_name(&adapter->bdaddr, name) < 0){
		DBG("fail!!, read device name: %s", name); 
		expand_name(name, MAX_NAME_LENGTH, main_opts.name,
							adapter->dev_id);
	}
	else {
		DBG("read device name: %s", name);    
	}
