...
// workaround for pin or key missing error while bonding (actually, opp sending has this issue)
// one more code in hci_event.c
	if (evt->status == 0x06) // 0x06-PIN or KEY MISSING 
	{
		char addr[18];

		info("err code:0x06(PIN or KEY MISSING), del linkkeys...");

		memset(addr, 0, sizeof(addr));
		ba2str(&dba, addr);
		delete_entry(sba, "linkkeys", (const char*)addr);
	}
// END SS_BLUEZ_BT
