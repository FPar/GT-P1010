...
//START:NCB-TI Changes for CSR: OMAPS00230025
    mQbufCount = 0;
    mSoftCocecThread = new SoftCodecThread(this);
    mSoftCocecThread->run("SoftCodecThread", PRIORITY_URGENT_DISPLAY);
...
//END:NCB-TI Changes for CSR: OMAPS00230025
    ...
    LOGV("DUMP_VIDEO_YUV construct fopen");
    fp_video_output = fopen("/data/video_out.yuv", "w+");
#endif
}
...
//START:NCB-TI Changes for CSR: OMAPS00230025
    ...
    if(neon_args)
    {
        free((NEON_CLRCNVRT_ARGS *)neon_args);
        neon_args = NULL;
    }
    if(pTIcnvr)
    {
        ...
        pTIcnvr = NULL;
        Neon_ClrCnvrt = NULL;
        LOGD("Unloaded NEON ColorConversion Library");
    }
#endif //NEON_OPT
    if(mSoftCocecThread != NULL)
    {
        Message msg;
        msg.command = THREAD_KILL;
        softcodecThreadCommandQ.put(&msg);
        softcodecThreadAckQ.get(&msg);
    }

    sp<SoftCodecThread> softcodecThread;

    { // scope for the lock
        Mutex::Autolock lock(mLock);
        softcodecThread = mSoftCocecThread;
    }

    // don't hold the lock while waiting for the thread to quit
    if (softcodecThread != 0)
    {
        softcodecThread->requestExitAndWait();
    }

    { // scope for the lock
        Mutex::Autolock lock(mLock);
        mSoftCocecThread.clear();
    }
//END:NCB-TI Changes for CSR: OMAPS00230025

#ifdef DUMP_VIDEO_YUV
    LOGV("DUMP_VIDEO_YUV destruct fclose ");
...
//START:NCB-TI Changes for CSR: OMAPS00230025
        mQbufCount = 0;
#ifdef NEON_OPT //NCB-TI
        pTIcnvr = dlopen("libclrcnvrt.so", RTLD_LAZY);
        if (!pTIcnvr) {
            LOGE("Open NEON ColorConversion Library Failed \n");
        }

        Neon_ClrCnvrt = (NEON_clrcnvt_fpo)dlsym(pTIcnvr, "Neon_ClrCnvrt");

        if ((error = dlerror()) != NULL) {
            LOGE("Couldnot find  Neon_ClrCnvrt symbol\n");
            dlclose(pTIcnvr);
            pTIcnvr = NULL;
        }
        else{
            if(!neon_args)
                neon_args   = (NEON_CLRCNVRT_ARGS*)malloc(sizeof(NEON_CLRCNVRT_ARGS));
        }
#endif //NEON_OPT
//END:NCB-TI Changes for CSR: OMAPS00230025
    }
   ...
   return mInitialized;
    ...
    if (mUseOverlay) {
		...
		// process of being destroyed.
                ...
                if(ref != NULL){
                    LOGV("Vendor Speicifc(34xx)MIO: overlay created ");
                    break;
                }
                else {
                    LOGE("Vendor Speicifc(34xx)MIO: Creating overlay failed");
                    usleep(20000);
                }
            }
...
//END:NCB-TI Changes for CSR: OMAPS00230025
            ...
            sWriteRespData[bufEnc].bInDSSQueue = false;
                ...
                bDequeueFail = false;
                bQueueFail = false;
                status = writeFrameBuf(aData, aDataLen, data_header_info, bufEnc); //NCB-TI Changes for CSR: OMAPS00230025
                 switch (status)
                 {
                     case PVMFSuccess:
     ...
     if(iEosReceived){
         ...
         if (!mConvert) //NCB-TI Changes for CSR: OMAPS00230025
         {
             WriteResponse resp(status,
...
#ifdef DUMP_VIDEO_YUV
    LOGV("DUMP_VIDEO_YUV writeFrameBuf fwrite");
    fwrite(aData,sizeof(uint8),aDataLen,fp_video_output);
#endif

    if (mUseOverlay == 0) return PVMFSuccess;
//START:NCB-TI Changes for CSR: OMAPS00230025
    if (mConvert) { //Software Codecs
        Message msg;
        bufEnc = aIndex;
        if((!sWriteRespData[bufEnc].bInDSSQueue)||(mQbufCount < 6))
        {
#ifdef NEON_OPT
            // Convert from YUV420 to YUV422 for software codec
            neon_args->pIn = (unsigned char*)aData;
            neon_args->pOut = (unsigned char*)mbufferAlloc.buffer_address[bufEnc];
            neon_args->width = iVideoWidth;
            neon_args->height = iVideoHeight;

            if (Neon_ClrCnvrt != NULL)
                (*Neon_ClrCnvrt)(neon_args);
            else
                LOGE("ColorConversion Fucntion pointer Null");
#else //NEON_OPT
            convertYuv420pToYuv422i(iVideoWidth, iVideoHeight, aData, mbufferAlloc.buffer_address[bufEnc]);
#endif // NEON_OPT

            //Send Buffer to Thread
            msg.command = THREAD_START;
            msg.arg1 = (void *)bufEnc;
            softcodecThreadCommandQ.put(&msg);
            mQbufCount++;
        }
        else
        {
            LOGV("AndroidSurfaceOutputOmap34xx::Buffer is with DSS %d Buffer Dropped",bufEnc);
        }
        // advance the overlay index if using color conversion
        if (++bufEnc == mbufferAlloc.maxBuffers) {
            bufEnc = 0;
        }
        if( !softcodecThreadAckQ.isEmpty() )
        {
            softcodecThreadAckQ.get(&msg);
            eStatus = (PVMFStatus) msg.arg1;
            mQbufCount--;
            return eStatus;
        }
        else //Default status
...
void AndroidSurfaceOutputOmap34xx::softcodecThread()
{
    Message msg;
    ...
    LOGD("Entering softcodecThread ++++\n");

    while(shouldLive)
    {
        //block for message
        softcodecThreadCommandQ.get(&msg);
        
        switch(msg.command)
        {
            case THREAD_START:
            {
                mQbufIndex = (int)msg.arg1;
                eStatus = displayFrame(mQbufIndex);
                if(eStatus == PVMFSuccess)
                    msg.command = THREAD_ACK;
                else
                    msg.command = THREAD_NACK;
                msg.arg1 = (void *) eStatus;
                softcodecThreadAckQ.put(&msg);
            }
            break;

            case THREAD_KILL:
            {
                LOGD("Receive Command: THREAD_KILL\n");
                err = 0;
                ...
                msg.command = THREAD_ACK;
                LOGD("Receive Command: THREAD_KILL %s\n", msg.command == THREAD_NACK ? "NACK" : "ACK"); 
    ...
    LOGD("Exiting softcodecThread ----\n");
}
PVMFStatus AndroidSurfaceOutputOmap34xx::displayFrame(int index)
{
    ...
    nActualBuffersInDSS = mOverlay->queueBuffer((void*)index);
 
    if(nActualBuffersInDSS < 0){
        LOGE("nextFrame(): mOverlay->queueBuffer(%d) failed",index);
        eStatus = PVMFErrArgument; // Queue failed
    }
    else // Queue succeeded
    {
        mBuffersQueuedToDSS++;
        sWriteRespData[index].bInDSSQueue = true;
        // This code will make sure that whenever a Stream OFF occurs in overlay...
        // a response is sent for each of the buffer.. so that buffers are not lost 
        if (mBuffersQueuedToDSS != nActualBuffersInDSS)
        {
            LOGE("nBuffers_queued_to_dss = %d, nOverlayBuffersQueued = %d", nActualBuffersInDSS, mBuffersQueuedToDSS);
            for(nReturnDSSBufIndex = 0; nReturnDSSBufIndex < mbufferAlloc.maxBuffers; nReturnDSSBufIndex++){
                if(nReturnDSSBufIndex == index){
                    LOGD("Skip this buffer as it is the current buffer %d", index);
                    continue;
                }
                if(sWriteRespData[nReturnDSSBufIndex].bInDSSQueue) {
                    LOGD("Sending dequeue response, %d", nReturnDSSBufIndex);
